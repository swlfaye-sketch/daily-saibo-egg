...
> | **每日自动监控** | **32 个** | 真正逐日抓取比对的部分。准确清单见 `scripts/watch_changes.py` 的 `TARGETS`。其中 2 个（MiniMax 开放平台、Flux 官方定价）为 2026-09-20 对视频与生图类 16 站逐站实测后新增。 |
...
| 2026-09-20 | **视频与生图类专项实测（16 站），监控数 30 → 32**。起因是评测者两次提到该类覆盖偏少。实测分两步：先判「能不能抓到可结构化比对的信号」，再判「该信号自身稳不稳定（连抓 3 次）」。结果：壳页 5 站（智谱清言 4 字／腾讯混元 4 字／通义万相 19 字／即梦定价页 24 字／可灵中国站 83 字）、不可达 6 站（Runway 308、Midjourney 403、Pika／Luma／Krea 代理 502、可图超时）、**「看着合格实则无用」3 站**（即梦／可灵／海螺正文 400–1100 字且连抓 3 次零波动，但既无价格也无档位名，只能做整页指纹，额度变化反映不出来）。**唯二过关的两站已收录入哨兵**（连抓 3 次指纹完全一致）。结论固化：判一个站能否进哨兵，**先看有无价格或档位这类结构化信号，再看稳不稳定，两条都过才收**。 |
